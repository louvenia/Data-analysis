...
В данном проекте создана база данных со знаниями об учебном заведении и написаны процедуры и функции для получения информации, а также процедуры и триггеры для её изменения.
...
## Логическое представление модели БД
...
По мере выполнения задания потребуются новые данные, чтобы проверить все варианты работы. 
Эти новые данные также добавлены в этом скрипте.

*Все задания должны быть названы в определенном формате, например A5_s21_memory. \
...
Если задан статус "начало", добавляется запись в таблицу Checks (в качестве даты используется текущий день). \
Добавляется запись в таблицу P2P. \
Если задан статус "начало", в качестве проверки указывается только что добавленная запись, иначе указывается проверка с незавершенным P2P этапом.
...
Добавляется запись в таблицу Verter (в качестве проверки указана проверка соответствующего задания с самым поздним (по времени) успешным P2P этапом)

##### 3) Написан триггер: после добавления записи со статутом "начало" в таблицу P2P, изменяется соответствующая запись в таблице TransferredPoints
...
##### 5) Посчитаны изменения в количестве пир поинтов каждого пира по таблице, возвращаемой [первой функцией из Part 3](#1-написана-функция-возвращающая-таблицу-transferredPoints-в-более-человекочитаемом-виде)
Результат выводится отсортированным по изменению числа поинтов. \
Формат вывода: ник пира, изменение в количество пир поинтов
...
Определение проводится исходя из рекомендаций друзей пира, т.е. находится пир, проверяться у которого рекомендует наибольшее число друзей. \
...
Для каждого месяца подсчитывается, сколько раз люди, родившиеся в этот месяц, приходили в кампус за всё время (будем называть это общим числом входов). \
Для каждого месяца считается, сколько раз люди, родившиеся в этот месяц, приходили в кампус раньше 12:00 за всё время (будем называть это числом ранних входов). \
Для каждого месяца просчитан процент ранних входов в кампус относительно общего числа входов. \
...
Для данной части задания создана отдельная база данных, которая заполнена таблицами, функциями, процедурами и триггерами, необходимыми для тестирования процедур (в файле *part4.sql*).

##### 1) Создана хранимая процедура, которая, не уничтожая базу данных, уничтожает все те таблицы текущей базы данных, имена которых начинаются с фразы 'TableName'.

##### 2) Создана хранимая процедура с выходным параметром, которая выводит список имен и параметров всех скалярных SQL функций пользователя в текущей базе данных. Имена функций без параметров не выводятся. Имена и список параметров должны выводиться в одну строку. Выходной параметр возвращает количество найденных функций.

##### 3) Создана хранимая процедура с выходным параметром, которая уничтожает все SQL DML триггеры в текущей базе данных. Выходной параметр возвращает количество уничтоженных триггеров.

##### 4) Создана хранимая процедура с входным параметром, которая выводит имена и описания типа объектов (только хранимых процедур и скалярных функций), в тексте которых на языке SQL встречается строка, задаваемая параметром процедуры.
